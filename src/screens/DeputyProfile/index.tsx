...
export const DeputyProfilScreen: React.FC<Props> = ({ route }) => {
  ...
  const getProcedureCountByDecision = (
    ...
    return procedures.reduce<{
      ...
      (prev, { decision }: { decision: 'YES' | 'ABSTINATION' | 'NO' | 'NOTVOTED' }) => {
  ...
  const votedProcedures = deputyData.matchesBar.map(({ procedure, decision }) => ({
